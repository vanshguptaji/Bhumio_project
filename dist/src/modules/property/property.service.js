"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var PropertyService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.PropertyService = void 0;
const common_1 = require("@nestjs/common");
const property_repository_1 = require("../../database/repositories/property.repository");
const offer_repository_1 = require("../../database/repositories/offer.repository");
const dto_1 = require("./dto");
let PropertyService = PropertyService_1 = class PropertyService {
    propertyRepository;
    offerRepository;
    logger = new common_1.Logger(PropertyService_1.name);
    constructor(propertyRepository, offerRepository) {
        this.propertyRepository = propertyRepository;
        this.offerRepository = offerRepository;
    }
    async create(createDto) {
        this.logger.log(`Creating new property: ${createDto.address}, ${createDto.city}`);
        const existing = await this.propertyRepository.findByAddressAndZip(createDto.address, createDto.zipCode);
        if (existing) {
            this.logger.log(`Property already exists at ${createDto.address}`);
            return dto_1.PropertyResponseDto.fromEntity(existing);
        }
        const property = await this.propertyRepository.save({
            address: createDto.address,
            city: createDto.city,
            state: createDto.state,
            zipCode: createDto.zipCode,
        });
        return dto_1.PropertyResponseDto.fromEntity(property);
    }
    async findAll() {
        this.logger.log('Fetching all properties');
        const properties = await this.propertyRepository.find({
            order: { createdAt: 'DESC' },
        });
        return dto_1.PropertyResponseDto.fromEntities(properties);
    }
    async findById(id) {
        this.logger.log(`Fetching property by ID: ${id}`);
        const property = await this.propertyRepository.findOne({
            where: { id },
        });
        if (!property) {
            throw new common_1.NotFoundException(`Property with ID ${id} not found`);
        }
        return dto_1.PropertyResponseDto.fromEntity(property);
    }
    async getDashboardData(propertyId) {
        this.logger.log(`Compiling dashboard data for property ID: ${propertyId}`);
        const property = await this.propertyRepository.findOne({
            where: { id: propertyId },
            relations: { disclosures: true },
        });
        if (!property) {
            throw new common_1.NotFoundException(`Property with ID ${propertyId} not found`);
        }
        const offers = await this.offerRepository.findByPropertyIdWithLoans(propertyId);
        const sortedOffers = (offers || []).sort((a, b) => {
            const scoreA = Number(a.strengthScore || 0);
            const scoreB = Number(b.strengthScore || 0);
            return scoreB - scoreA;
        });
        return {
            ...dto_1.PropertyResponseDto.fromEntity(property),
            disclosures: property.disclosures || [],
            offers: sortedOffers,
        };
    }
};
exports.PropertyService = PropertyService;
exports.PropertyService = PropertyService = PropertyService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [property_repository_1.PropertyRepository,
        offer_repository_1.OfferRepository])
], PropertyService);
//# sourceMappingURL=property.service.js.map